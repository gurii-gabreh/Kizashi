...
バインディング）を使う。以下2点は2026-08-15にネット検索で確認済み：

1. **`AI_MODEL_ID`**（`chat.js`内、`@cf/meta/llama-3.1-8b-instruct`）は
   Cloudflareの現行カタログに引き続き存在することを確認済み
2. **`worker/lib/crisisResponse.js`の相談窓口電話番号**（よりそいホットライン、
   いのちの電話）を確認したところ、「いのちの電話」の番号が誤っていた
   （`0570-064-556` → 正しくは`0570-783-556`）ため修正済み。よりそい
   ホットライン（`0120-279-338`）は正しかった。両窓口とも受付時間を
   合わせて表示するようにした

番号は今後変更される可能性があるため、長期間運用する場合は定期的な
再確認が必要（誤った番号は危機的状況の相手に実害を与えるため）。
